$(document).ready(function(){
	...
	*/
	...
	$(".add-info").on("click", function(){
		...
		$.ajax({
			url : 'action.php',
			method : 'POST',
			data : $("#add-infor-form").serialize(),
			success : function(response){
				var resp = $.parseJSON(response);
				if (resp.status == 202) {
					getBrands();
					$("#add_infor_modal").modal('hide');
					alert(resp.message);
	...
	*/
	function net_total(){
		...
		$('.total').each(function(){
			net_total += ($(this).val()-0)/dolla;//usd
			// net_total1 += ($(this).val()-0);//vnd
